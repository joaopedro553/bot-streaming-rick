...
# Serviços que o bot gera arquivo .txt na hora para o cliente
COOKIES_FILES = ['prime', 'youtube', 'canva']
# Serviços especiais (Links ou blocos)
...
# --- GESTÃO THOMAS (PRIVADO - PRIORIDADE 1) ---
...
def clear_db(message):
    ...
    try:
        ...
        if s_limpar in ALL_SERVICES:
            ...
            bot.reply_to(message, f"🗑️ *Thomas, o banco de {s_limpar.upper()} foi zerado!*", parse_mode='Markdown')
    except: pass

# Abastecer via Arquivo (Lê IPTV por blocos e outros por linhas)
...
def handle_upload(message):
    ...
    if serv in ALL_SERVICES:
        ...
        if serv == 'iptv':
            # Inteligência: Separa os hits de IPTV pela divisória de traços
            hits = content.split('--------------------------------------------------')
            docs = [{"dados": h.strip()} for h in hits if len(h.strip()) > 10]
        else:
            # Streaming e Cookies: 1 hit por linha
...
if __name__ == "__main__":
    ...
    bot.remove_webhook()
    print("🚀 Bot Thomas V4 - Inteligência de Arquivos Ativada!")
